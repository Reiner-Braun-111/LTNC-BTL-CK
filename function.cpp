...
void mineManager()
{
    ...
    if ( !gameOver && !isWinning )
    {
        ...
        //Render text
        gMineLeftTexture.render( 5, SCREEN_HEIGHT - gMineLeftTexture.getHeight()-10 );
    }
}
...
void timeManager(){
    ...
    if ( !gameOver && !isWinning )
    {
    	...
    	//Render text
        gTimeTexture.render( ( 5+  gMineLeftTexture.getWidth() + 10) , SCREEN_HEIGHT - gTimeTexture.getHeight()-10 );
...
bool loadMedia()
{
    ...
    gName = TTF_OpenFont( "Font/whitrabt.ttf", 80 );
    if( gName == NULL )
	{
	    cout << "Failed to load whitrabt font! SDL_ttf Error: " << TTF_GetError() << endl;
		success = false;
	...
	else
    {
		...
		if( !gNameTexture.loadFromRenderedText( "MINESWEEPER", playAgainLose ) )
		{
			cout << "Failed to render text texture!\n";
			success = false;
		}
    ...
    if( !gMenuItemTexture.loadFromFile( "Image/Screenshot 2022-04-23 185619.png" ) )
	{
		cout << "Failed to load sprites texture!\n";
		success = false;
	}
	else
    {
		...
		{
			gMenuItem[ i ].x = i*300 + 10 ; 
			gMenuItem[ i ].y = 0 ;
			gMenuItem[ i ].w = menuItemW ;
			gMenuItem[ i ].h = menuItemH;
		}
		//Set buttons position
		for (int i = 0; i < menuItemTotal; i++)
        {
            
            gMenuButtons[i].setPosition(80, i*50 +200 );
	...
	while(!notExit){
		...
		while( SDL_PollEvent(&e) != 0 ){
			...
			if ( e.type == SDL_MOUSEBUTTONDOWN )
 		   {
				...
				for (int i=0; i < menuItemTotal; i++)
  			    {
					if ((mouseX >  gMenuButtons[i].mPosition.x) && (mouseX < gMenuButtons[i].mPosition.x + menuItemW) && (mouseY > gMenuButtons[i].mPosition.y) && (mouseY < gMenuButtons[i].mPosition.y + menuItemH)){
   	     				j=i;
						notExit=true;
    				}
		...
		gNameTexture.render( ( SCREEN_WIDTH - gNameTexture.getWidth() ) / 2, 100 );

        for( int i = 0; i < menuItemTotal; i++ )
        {
            gMenuItemTexture.render(80, i*50 +200,  &gMenuItem[ i ] );
        }
